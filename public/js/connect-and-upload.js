...
function handleFileSelect(evt) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (files_checksum.includes(fileChecksum(files[i]))) {
            ...
            files[i].size + ' bytes, last modified: ' + files[i].lastModifiedDate.toLocaleDateString() +'</spam></li>';
...
function updateList (checksum, ipfsHash) {
  ...
  '</a></spam><br> <spam id="fileProperties"> (' + (filesOk[i].type || 'n/a' ) +') - ' + filesOk[i].size + ' bytes, last modified: ' + filesOk[i].lastModifiedDate.toLocaleDateString() +'</spam>'
}
...
function fileChecksum(file) {
  ...
  var checksum = MD5(file.name + file.size + file.lastModifiedDate)
  return checksum
}
